...
export class ModalStoryComponent  {
  ...
  selectAudience(audience: string) {
    this.selectedAudience = audience; // تحديث القيمة عند اختيار المستخدم
  }
  

  saveAudience() {
    console.log('Audience saved:', this.selectedAudience);
  }

  // إضافة إيموجي للنص
  addEmoji(event: any) {
    console.log('Emoji added:', event.emoji);
    // يمكنك إضافته إلى نص الإدخال هنا
  }

  // سحب الصورة إلى المنطقة المخصصة
